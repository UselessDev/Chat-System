...
public class ChatGUI extends JFrame {
    ...
    private void appendMessage(Message msg) {
        ...
        switch (msg.getType()) {
            case SYSTEM:
                formatted = "[SYSTEM] " + msg.getContent();
                color = COLOR_SYSTEM;
                break;
            case PRIVATE:
                formatted = "[DM] " + msg.getSender() + " -> " + msg.getRecipient() + ": " + msg.getContent();
                color = new Color(128, 0, 128); // Purple for private
                break;
            default: // BROADCAST
                if (msg.getSender().equals(client.getUsername())) {
                    formatted = "You: " + msg.getContent();
                    color = new Color(0, 100, 200); // Blue for own messages
                } else {
                    formatted = msg.getSender() + ": " + msg.getContent();
                }
        }
